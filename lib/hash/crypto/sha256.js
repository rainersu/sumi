...
function binb_sha256 (m, l) {
	...
	for(x =m.length; i < x; i+= 16) {
		...
		h = H[7];
		for(j = 0; j < 64; j++) {
			W[j] = j < 16 ? m[j + i] : A(A(A(sha256_Gamma1256(W[j - 2]), W[j - 7]), sha256_Gamma0256(W[j - 15])), W[j - 16]);
			T1 = A(A(A(A(h, sha256_Sigma1256(e)), sha256_Ch(e, f, g)), sha256_K[j]), W[j]);
			T2 = A(sha256_Sigma0256(a), sha256_Maj(a, b, c));
			h = g;
			g = f;
			f = e;
			e = A( d, T1);
...
*
* console.log(Crypto.sha256('abc', 'key'));        
