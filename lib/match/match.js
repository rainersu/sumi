...
function N (n, o) {
	return P[n]  = 
		type(o)  === 'array' ? 
		new Match(o[0], o[1], o[2]) : 
		(o =  Object(o), new Match(o.pattern, o.keys, o.parser));
}

function B (n) {
	for (var i in n) if (hasOP.call(n, i)) {
		N(i, n[i]);
	}
	return Match;
}
...
/**
* 快速创建本类型的一个实例并将它添加为 {@link Sumi.Pattern Pattern} 的静态成员。
* @see {@link Sumi.Match.batch}
* @access public
* @func Sumi.Match.create
* @param {string} name - 用作字段名的新建实例名
* @param {object|array} params - 传递给构造函数的参数。参见下面示例
* @returns {Sumi.Match}
* @example
* var rex = Sumi.Match.create('email', [ 
*     '([-0-9a-zA-Z.+_]+)@([-0-9a-zA-Z.+_]+\\.[a-zA-Z]{2,4})', 
 *    'user,domain' 
* ]);
* 
* console.log(rex.parse('admin@me.com'));
...
/**
* 批量创建本类型的实例，并将它们添加为 {@link Sumi.Pattern Pattern} 的静态成员。
* @see {@link Sumi.Match.create}
* @access public
* @func Sumi.Match.batch
* @param {object} params - 用于创建实例的参数键值对。参见下面示例
* @returns {object} {@link Sumi.Match}
* @example
* Sumi.Match.batch({
*     'email', [ 
*         '([-0-9a-zA-Z.+_]+)@([-0-9a-zA-Z.+_]+\\.[a-zA-Z]{2,4})', 
...
*     'number', [
*         '([+\\-]?\\d*)(?:\\.(\\d*))?(?:[eE]([+\\-]?\\d+))?',
*         'int,decimal,exponential'	
*     ]
* });
* 
* console.log(Sumi.Pattern.email);
* // { 
* //     source: '([-0-9a-zA-Z.+_]+)@([-0-9a-zA-Z.+_]+\\.[a-zA-Z]{2,4})',
* //     keys: [ 'match', 'user', 'domain' ],
* //     parser: undefined
* // }
*/
